...
export const register = async (req, res) => {
  try {
    ...
    // send verification email asynchronously so registration is fast
    verifyEmail(token, email)
      .then((sent) => {
        if (!sent) console.warn('Registration: verification email not sent')
      })
      .catch((e) => console.warn('Registration: verifyEmail failed', e?.message || e))
    newUser.token = token
    await newUser.save()
...
export const login = async (req, res) => {
  try {
    ...
    if (existingUser.isVerified === false) {
      return res.status(400).json({
        success: false, message: "Verify your account then login"
      })
    }
    ...
    //Checking for existing session and validate
...
export const verifyOTP = async (req, res) => {
  try {
    ...
    if (!user.otp || !user.otpExpiry) {
      return res.status(400).json({
        success: false, message: "Otp is not generated or already verified"
      })
    }
    ...
    if (otp !== user.otp) {
      return res.status(400).json({
        success: false, message: "Invalid OPT!"
      })
    }
    user.otp = null;
    user.otpExpiry = null;
...
export const changePassword = async (req, res) => {
  try {
    ...
    if (!newPassword || !confirmPassword) {
      return res.status(400).json({
        success: false, message: "Please enter new password and confirm"
      })
    }
    if (newPassword !== confirmPassword) {
      return res.status(400).json({
